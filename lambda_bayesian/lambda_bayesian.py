...
MODEL_CONFIG = {
    ...
    "signal_thresholds": {
        ...
        "SELL": {"prob_up_below": 0.35, "rationale": "Alta confianza bajista / Salir a Cash"},
        "HOLD": {"range": [0.35, 0.65], "rationale": "Incertidumbre elevada / Salir a Cash"}
    },
    ...
    "cpt_market_direction": {
        ...
        "values_P_down": [
            0.15, 0.25, 0.30, 0.20, 0.30, 0.35, 0.30, 0.40, 0.10, 0.15, 0.45, 0.50,
            0.70, 0.75, 0.80, 0.75, 0.80, 0.85, 0.80, 0.85, 0.50, 0.55, 0.90, 0.95,
            0.45, 0.50, 0.55, 0.50, 0.55, 0.60, 0.55, 0.60, 0.25, 0.30, 0.65, 0.70
        ],
        "values_P_up": [
            0.85, 0.75, 0.70, 0.80, 0.70, 0.65, 0.70, 0.60, 0.90, 0.85, 0.55, 0.50,
...
def discretize_volatility(bb_upper, bb_lower, close_price):
    if bb_upper is None or bb_lower is None: return "low", 0.0
    try:
        if np.isnan(float(bb_upper)) or np.isnan(float(bb_lower)): return "low", 0.0
    except: return "low", 0.0
...
def create_bayesian_network():
    ...
    cpd_s = TabularCPD('Sentiment', 3, [[priors["Sentiment"]["bullish"]], [priors["Sentiment"]["bearish"]], [priors["Sentiment"]["neutral"]]], state_names={'Sentiment': ['bullish', 'bearish', 'neutral']})
    cpd_r = TabularCPD('RSI', 3, [[priors["RSI"]["oversold"]], [priors["RSI"]["neutral"]], [priors["RSI"]["overbought"]]], state_names={'RSI': ['oversold', 'neutral', 'overbought']})
    cpd_t = TabularCPD('Trend', 2, [[priors["Trend"]["uptrend"]], [priors["Trend"]["downtrend"]]], state_names={'Trend': ['uptrend', 'downtrend']})
    cpd_v = TabularCPD('Volatility', 2, [[priors["Volatility"]["low"]], [priors["Volatility"]["high"]]], state_names={'Volatility': ['low', 'high']})
    ...
    cpd_d = TabularCPD(
        ...
        state_names={'MarketDirection': ['down', 'up'], 'Sentiment': ['bullish', 'bearish', 'neutral'], 'RSI': ['oversold', 'neutral', 'overbought'], 'Trend': ['uptrend', 'downtrend'], 'Volatility': ['low', 'high']}
    )
...
def infer_signal(model, evidence_states):
    ...
    result = infer.query(variables=['MarketDirection'], evidence=evidence_states, show_progress=False)
...
def get_ticker_data(connection, target_date, ticker):
    ...
    cursor.execute("SELECT sentiment, confidence, headline, justification FROM sentiment_scores WHERE batch_date = %s AND ticker = %s ORDER BY confidence DESC", (target_date, ticker))
    all_sentiments = cursor.fetchall()
    cursor.execute("SELECT rsi_14, sma_20, sma_50, close_price, bb_upper, bb_lower FROM technical_indicators WHERE batch_date = %s AND ticker = %s LIMIT 1", (target_date, ticker))
    indicators = cursor.fetchone()
    cursor.close()
    return all_sentiments, indicators

def aggregate_sentiment(all_sentiments):
    if not all_sentiments: return None, None, {}
    ...
    headlines_sample = [{"headline": row[2][:120] + "..." if len(row[2]) > 120 else row[2], "sentiment": row[0], "confidence": round(float(row[1]), 4)} for row in all_sentiments[:10]]
    return dominant_sentiment, dominant_confidence, {
        "total_headlines": total, "aggregation_method": "max_confidence", "distribution": distribution,
        "dominant": {"sentiment": dominant_sentiment, "confidence": dominant_confidence}, "headlines_sample": headlines_sample,
...
def save_bayesian_trace(batch_date, tickers_trace, execution_meta):
    trace = {
        "schema_version": "2.0", "batch_date": batch_date, "generated_at": datetime.now(timezone.utc).isoformat(),
        "execution": execution_meta, "model_config": MODEL_CONFIG, "tickers": tickers_trace,
        "audit_notes": {"cpt_source": "Parametros ajustados para capturar momentum alcista", "threshold_rsi": "RSI <30 = oversold, >70 = overbought", "threshold_vol": "BB width ratio >0.05 = high", "threshold_signal": "P(up) >0.65 = BUY, <0.35 = SELL", "known_issues": MODEL_CONFIG["known_limitations"]}
    }
    key = f"results/{batch_date}/bayesian_trace.json"
    s3_client.put_object(Bucket=DATALAKE_BUCKET, Key=key, Body=json.dumps(trace, indent=2, default=str), ContentType="application/json")
    return key
...
def upsert_signal_explanation(connection, batch_date, ticker, states):
    ...
    """, (batch_date, ticker, states["Sentiment"], states["RSI"], states["Trend"], states["Volatility"]))
    connection.commit()
    cursor.close()
...
def handler(event, context):
    ...
    try:
        ...
        if not latest_date: return {'statusCode': 200, 'body': 'No data'}
        ...
        for ticker in tickers:
            ...
            try:
                ...
                if not all_sentiments or not indicators_result:
                    skipped.append({"ticker": ticker, "reason": "incomplete_data"}); continue
                ...
                if dominant_sentiment is None:
                    skipped.append({"ticker": ticker, "reason": "no_sentiment"}); continue

                evidence_states = {"Sentiment": discretize_sentiment(dominant_sentiment), "RSI": discretize_rsi(float(rsi_14)), "Trend": discretize_trend(float(sma_20), float(sma_50)), "Volatility": vol_state}
                ...
                tickers_trace[ticker] = {
                    ...
                    "discretization": {"sentiment_raw": dominant_sentiment, "sentiment_conf": dominant_confidence, "sentiment_state": evidence_states["Sentiment"], "rsi_state": evidence_states["RSI"], "trend_state": evidence_states["Trend"], "volatility_state": evidence_states["Volatility"]},
                    "sentiment_detail": sentiment_detail,
                    "inference": {"prob_up": prob_up, "prob_down": prob_down, "signal": signal, "threshold_used": (MODEL_CONFIG["signal_thresholds"]["BUY"]["prob_up_above"] if signal == "BUY" else MODEL_CONFIG["signal_thresholds"]["SELL"]["prob_up_below"] if signal == "SELL" else MODEL_CONFIG["signal_thresholds"]["HOLD"]["range"])},
                    "reasoning": reasoning
                }
                signals_processed += 1
        ...
        logger.error(f"Critical error: {e}"); return {'statusCode': 500, 'body': json.dumps({'error': str(e)})}