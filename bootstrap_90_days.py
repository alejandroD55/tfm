...
def create_bayesian_network():
    try:
        ...
        cpd_direction = TabularCPD(
            ...
            state_names={
                'MarketDirection': ['down', 'up'], 'Sentiment': ['bullish', 'bearish', 'neutral'],
                'RSI': ['oversold', 'neutral', 'overbought'], 'Trend': ['uptrend', 'downtrend'],
                'Volatility': ['low', 'high']
            }
        )
...
# =================================================================
# 2. CONEXIÓN A BASE DE DATOS BLINDADA
# =================================================================
def get_db_connection():
    return psycopg2.connect(host=DB_HOST, port=DB_PORT, user=DB_USER, password=DB_PASS, database=DB_NAME)

def ensure_connection(conn):
    """ Función salvavidas que reconecta si la conexión se ha caído por inactividad """
    try:
        with conn.cursor() as cursor:
            cursor.execute("SELECT 1")
        return conn
    except (psycopg2.OperationalError, psycopg2.InterfaceError):
        print("\n🔄 Conexión perdida con AWS Aurora. Reconectando de forma automática...")
...
def run_bootstrap():
    ...
    for ticker in TICKERS:
        ...
        for date, row in tqdm(df.iterrows(), total=len(df), desc=f"Simulando {ticker}"):
            current_date_str = date.strftime('%Y-%m-%d')
            
            # --- 0. COMPROBAR CONEXIÓN ANTES DE ACTUAR ---
            conn = ensure_connection(conn)
            
            # --- 1. GUARDAR INDICADORES ---
            try:
                # Usamos el cursor de usar y tirar dentro del contexto "with"
                with conn.cursor() as cursor:
                    cursor.execute("""
            ...
            # --- 2. DESCARGAR NOTICIAS Y VOTACIÓN MAYORITARIA ---
            ...
            if news_resp and isinstance(news_resp, list):
                sentiments_list = []
                top_article_saved = False 
                
                for article in news_resp[:5]:
                    headline = article.get('headline', '')
                    if headline:
                        try:
                            res = hf_client.text_classification(headline, model=MODEL_ID)
                            ...
                            clasificacion = sentiment_map.get(label, 'neutral')
                            sentiments_list.append(clasificacion)
                            
                            if not top_article_saved:
                                conn = ensure_connection(conn)
                                try:
                                    with conn.cursor() as cursor:
                                        cursor.execute("""
                                        ...
                                        """, (current_date_str, ticker, headline, clasificacion))
                                    conn.commit()
                                    top_article_saved = True
                                except Exception as e:
                                    try: conn.rollback()
                                    except Exception: pass
                        except Exception:
                ...
                if sentiments_list:
                    daily_sentiment = max(set(sentiments_list), key=sentiments_list.count)
    ...
    conn.close()
    print("\n✅ BACKFILL COMPLETADO. ¡Tu base de datos ahora tiene 1 año de historia limpia y sin sesgos!")
